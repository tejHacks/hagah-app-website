  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-16 md:px-6 md:py-24">
      <header className="text-center">
        <p className="text-sm font-medium uppercase tracking-widest text-primary">Download {'&'} Test</p>
        ...
        <p className="mx-auto mt-4 max-w-xl text-lg leading-relaxed text-pretty text-muted-foreground">
          Download the latest APK and install it directly. iOS support is on the way.
        </p>
      </header>
      ...
      <div className="mt-10 flex flex-col items-center gap-4 rounded-3xl border border-border bg-card px-6 py-12 text-center">
        ...
        <div>
          <p className="font-serif text-2xl font-semibold text-foreground">Hagah for Android</p>
          <p className="mt-1 text-sm text-muted-foreground">Version 1.0.0 &middot; APK &middot; ~18 MB</p>
        </div>
        <Button size="lg" className="mt-2 gap-2" render={<a href={apkUrl} download />}>
      ...
      <section className="mt-16">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {steps.map((s) => (
            <div key={s.step} className="flex gap-4 rounded-2xl border border-border bg-card p-5">
        ...
        <p className="mt-6 text-center text-xs leading-relaxed text-muted-foreground">
          Installing an APK from outside the Play Store is safe when it comes from a source you
          trust. You can disable {'"'}unknown sources{'"'} again after installing.
        </p>
      </section>
